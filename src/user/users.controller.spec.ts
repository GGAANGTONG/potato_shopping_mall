...
describe('UserController', () => {
  ...
  it('1-1. User - register, signUpDto를 정상적으로 전달 받아 회원가입에 성공함', async () => {
  ...
  it('2-1. User - signIn, signInDto를 정상적으로 전달 받아 로그인에 성공함', async () => {
  ...
  it('4-2. User - findOne, 매개변수로 입력받은 id에 해당하는 값이 할당되지 않은 채로 전달되면 user 정보를 반환하지 않음', async () => {
    ...
    await controller.findOne(id).catch((err) => {
      expect(err).toBe(err);
    });
  });

  it('5-1. User - update, updateDto를 정상적으로 전달 받아 업데이트에 성공함', async () => {
  ...
  it('5-2. User - update, 매개변수로 입력받은 id에 해당하는 값이 할당되지 않은 채로 전달되면 에러를 반환함', async () => {
    const updateDto: UpdateDto = {
      nickname: 'gookbab95@gmail.com',
      profile: '좀 더 맛있는 국밥 사진',
      password: 'Ex@mple1234567!!!',
  ...
  it('6-1. User - delete, id를 정상적으로 전달 받아 삭제에 성공함', async () => {
    const id: number = 1;
  ...
  it('6-2. User - delete, 매개변수로 입력받은 id에 해당하는 값이 할당되지 않은 채로 전달되면 삭제에 실패하고 에러를 반환함', async () => {
    const id: number = 1;
    ...
    await controller.remove(id).catch((err) => {
      expect(err).toBe(err);
    });
    // expect(result).toStrictEqual({
    //   message: '삭제 되었습니다'
