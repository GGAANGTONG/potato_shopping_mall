...
export class OrdersController {
  ...
  purchase(@Request() req, @Body() createOrderDto: CreateOrderDto) {
    ...
    logger.traceLogger(
      'Orders - purchase',
      `req.user = ${JSON.stringify(req.user)}, createOrderDto = ${JSON.stringify(createOrderDto)} `,
    );
  ...
  @UseGuards(AuthGuard('jwt'))
  @Get('today-count')
  async getTodayOrdersCount(@Request() req) {
    const userId = req.user.id;
    const count = await this.ordersService.getTodayOrdersCount(userId);
    return count;
  }
