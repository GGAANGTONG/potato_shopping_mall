...
describe('CommentsService', ()=> {
  ...
  it('1-1. Comments - create, userId & createCommentDto를 정상적으로 전달받아 댓글을 생성함', async ()=> {
    ...
    commentsRepository.create.mockReturnValueOnce(mockReturn)
    commentsRepository.save.mockResolvedValueOnce(mockReturn)
  ...
  it('2-1. Comments - findAllByUserId, 정상적으로 특정 유저의 모든 댓글을 조회함',async ()=> {
    ...
    commentsRepository.findBy.mockResolvedValueOnce(mockedReturn)
  ...
  it('2-2. Comments - findAllByUserId, 정상적으로 특정 유저의 특정 게시글의 모든 댓글을 조회함',async ()=> {
    ...
    commentsRepository.find.mockResolvedValueOnce(mockedReturn)

    return await expect(service.findAllByUserId(userId, board_id)).resolves.toBe(mockedReturn)
  })

  it('2-3. Comments - findAllByUserId, userId에 0 || null || undefined가 할당된 채로 전달받아 에러를 반환함', async ()=> {
    const userId = 0
    const board_id = 1
  ...
  it('4-1. Comments - update, userId & updateBoardDto를 정상적으로 전달받아 댓글을 생성함', async ()=> {
    ...
    commentsRepository.findOne.mockResolvedValueOnce(returnOriginalValue)
    ...
    commentsRepository.create.mockReturnValueOnce(mockReturn1)
    commentsRepository.update.mockReturnValueOnce(mockReturn1)
    ...
    return await expect(service.update(userId, updateCommentDto)).resolves.toEqual(mockReturn2)

  })

  it('4-2. Comments - update, userId에 0 || null || undefined가 할당된 채로 전달받아 에러를 반환함', async ()=> {
    const userId = 0
    const updateCommentDto:UpdateCommentDto = {
      board_id: 1,
      content: '죽음의 국밥이군요'
  ...
  it('4-3. Comments - update, updateCommentDto가 불완전한 상태로 전달받아 에러를 반환함(board_id == 0인 경우 포함)', async ()=> {
  ...
  commentsRepository.update.mockResolvedValueOnce(mockReturn)
  ...
  it('5-1. Comments - remove, req와 board_id를 전달받아 정상적으로 특정 유저의 특정 댓글을 삭제함',async ()=> {
    ...
    commentsRepository.findOne.mockResolvedValueOnce(mockedReturn0)
    commentsRepository.delete.mockResolvedValueOnce(mockedReturn1)
    ...
    return await expect(service.remove(userId, commentId)).resolves.toEqual(mockedResult)
    
  })

  it('5-2. Comments - remove, userId, commentId 중 하나 이상에 0 || undefined || null이 할당된 채로 전달받아 에러를 반환함',async ()=> {
    const userId = 0
    const commentId = 1
