...
describe('CategoriesService', () => {
  ...
  it('1-1. Categories - create, createCategoryDto를 전달받아 카테고리 정보를 생성함', async () => {
    ...
    return await expect(service.create(createCategoryDto)).resolves.toBe(returnedValue)
  })
  ...
  it('1-2. Categories - create, createCategoryDto가 불완전한 상태로 전달받아 에러를 반환함', async () => {
    ...
    return await validation(CreateCategoryDto, createCategoryDto).catch(err => expect(err))
  ...
  it('4-1. Categories - update, id와 createCategoryDto를 전달받아 카테고리 정보를 갱신함', async () => {
    ...
    return await expect(service.update(id, createCategoryDto)).resolves.toBe(returnedUpdatedValue)
  })
  ...
  it('4-2. Categories - update, id에 null/undefined가 할당된채 전달받아 에러를 반환함', async () => {
    ...
    return await expect(service.update(id, createCategoryDto)).rejects.toThrow(returnedOriginalValue)
  })
