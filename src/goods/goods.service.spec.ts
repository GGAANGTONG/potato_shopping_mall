...
import { ValidationPipe, ArgumentMetadata, NotFoundException } from '@nestjs/common';
...
describe('GoodsService', () => {
  ...
  let categoriesRepository: Partial<Record<keyof Repository<Categories>, jest.Mock>>;
  ...
  it('1-2. Goods - create, createDto가 불완전한 상태로 전달받아 에러를 반환함, 근데 이거 프로덕션 이후에는 category에 문자가 들어오는 걸 막을 방법이 있나?', async () => {
    ...
    return await validation(CreateGoodDto, createGoodDto).catch(
      err => expect(err)
      )
  ...
  it('1-3. Goods - create, createDto를 전달받았으나, cate_id에 해당하는 카테고리 데이터가 없어  에러를 반환함', async () => {
    ...
    return await expect(service.create(createGoodDto)).rejects.toThrow(NotFoundException)
  })
  ...
  it('4-1. Goods - update, id와 updateGoodDto 전달받아 상품 정보를 갱신함(이것도 dto extends를 picktype에서 partial로 바꿔야 할 거임)', async () => {
    ...
    return await expect(service.update(id, updateGoodDto)).resolves.toBe(value2)
  })
  ...
  it('4-2. Goods - update, id에 undefined/null이 할당된채 전달받아 상품을 찾지 못해 에러를 반환함', async () => {
    ...
    return await expect(service.update(id, updateGoodDto)).rejects.toThrow(NotFoundException)
  })
