...
export class RackService {
  ...
  async addGoodsToRack(addGoodsToRackDto: AddGoodsToRackDto): Promise<Racks> {
    ...
    try {
      return await this.racksRepository.manager.transaction(async entityManager => {
        ...
        if (!goods) {
          throw new NotFoundException('해당 상품 정보를 찾을 수 없습니다.');
        }
        ...
        if (existingGoodsInRack) {
          throw new BadRequestException('이미 해당 랙에 동일한 상품이 존재합니다.');
        }
      ...
      throw new InternalServerErrorException('랙에 상품 적재 중 문제가 발생했습니다.', error.message);
    }
  }
  ...
  async update(id: number, updateRacksDto: UpdateRacksDto) {
    ...
    try {
      ...
      throw new InternalServerErrorException(`창고 정보 업데이트 중 문제가 발생했습니다.`);
    }
  }
  ...
  async remove(id: number) {
    ...
    try {
      ...
      throw new InternalServerErrorException('랙 정보 삭제 중 문제가 발생했습니다.');
    }
  }
}
